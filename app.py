...
if not region and not state and not city:
    ...
    filtered_df = df[df["Region"].isin(state) & df3["City"].isin(city)]
elif state and city:
    filtered_df = df[df["Region"].isin(state) & df3["State"].isin(city)]
elif city:
...
with cl2:
    with st.expander("Region_ViewData"):
        ...
        st.write(region.style.background_gradient(cmap=cmap))
        csv = category_df.to_csv(index = False).encode('utf-8')
